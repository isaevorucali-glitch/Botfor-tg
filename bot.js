...
async function showDashboard(chatId) {
    ...
    // Топ-5 рефералов для дашборда
    let topReferralsText = "";
    const topReferrals = Array.from(referralCounts.entries())
        .sort((a, b) => b[1] - a[1])
        .slice(0, 5);
    if (topReferrals.length > 0) {
        ...
        for (let i = 0; i < topReferrals.length; i++) {
            ...
            topReferralsText += `${i+1}. ${user.firstName || uid} — ${count} рефералов\n`;
        }
    }
    
    const dashboardText = `📊 *ДАШБОРД БОТА*\n\n` +
        `👥 *ПОЛЬЗОВАТЕЛИ*\nВсего: ${totalUsers}\nАктивных (с покупками): ${ratings.size}\n\n` +
            ...
            `После подписки нажми кнопку 👇`,
            {
                ...
                reply_markup: {
                    inline_keyboard: [
                        ...
                        [{ text: '✅ ПРОВЕРИТЬ', callback_data: 'check_subscribe' }]
                    ]
                }
            }
        );
...
bot.onText(/\/referrals/, (msg) => {
    ...
    const balance = getBalance(userId);
    const rank = getReferralRank(userId);
    const botUsername = bot.options.username || 'bot_username';
...
bot.on('message', async (msg) => {
    ...
    if (text.includes('помощ') || text.includes('помоги') || text === 'help') {
        const helpText = `❓ *ЧЕМ ПОМОЧЬ?*\n\n` +
            `📋 *КОМАНДЫ:*\n` +
            `/menu — Главное меню\n` +
            `/profile — Мой профиль\n` +
            `/balance — Мой баланс\n` +
            ...
            `/faq — Частые вопросы\n\n` +
            `📞 Связаться: @${MANAGER_USERNAME}`;
        bot.sendMessage(msg.chat.id, helpText, { parse_mode: 'Markdown' });
        return;
    }
    
    if (text.includes('как купить') || text.includes('купить подарок')) {
        bot.sendMessage(msg.chat.id,
            `🎁 *КАК КУПИТЬ ПОДАРОК*\n\n` +
            `1. Нажми /menu\n` +
    ...
    if (text.includes('спасиб') || text.includes('благодар')) {
        bot.sendMessage(msg.chat.id, `Всегда рады помочь! 💝\n\nЕсли есть вопросы — пиши @${MANAGER_USERNAME}`);
        return;
    }
    
    if (text.includes('реферал') || text.includes('пригласить')) {
        const botUsername = bot.options.username || 'bot_username';
        const refLink = `https://t.me/${botUsername}?start=${userId}`;
        const refCount = referralCounts.get(userId) || 0;
        bot.sendMessage(msg.chat.id,
            ...
            `${refLink}\n\n` +
            `За каждого друга ты получишь +5 руб на баланс!\n\n` +
...
bot.on('callback_query', async (callbackQuery) => {
    ...
    if (data === 'buy_bear' || data === 'buy_romantic') {
        ...
        if (balance >= giftPrice) {
            ...
            keyboard.push([{ text: '💳 ОПЛАТИТЬ МЕНЕДЖЕРУ', callback_data: `pay_manager_${data}` }]);
            keyboard.push([{ text: '💸 ПОПОЛНИТЬ БАЛАНС', callback_data: 'menu_referrals' }]);
        }
